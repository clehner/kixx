//this.debug = require("debug", "0.1");
//this.repr = debug.repr;
//this.kdump = debug.kdump;
...
function TestSuite_run(aCallback)
{
  ...
  let cases = Iterator(this._cases);
...
let TestSimpleVocab = 
{
  ok: function TestSimpleVocab_ok(callback)
  {
    function ok(assertion, desc, diag)
    {
      ...
      let passed = assertion ? true : false;
      callback({
          passed: passed,
          description: desc,
          diagnostic: diag
...
let DumpOutputFormatter =
{
  ...
  endSuite: function DOF_endSuite(result)
  {
    ...
    for(let name in result.cases)
    {
      let cas = result.cases[name];
      dump("\t-> test case: "+ name +"\n");

      let tests = result.cases[name].tests;
      let failures = [t for each(t in tests) if(t.result != "ok")];
